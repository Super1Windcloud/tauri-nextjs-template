...
export function WindowTitlebar() {
	const currentWindow = useMemo<Window | null>(() => {
		...
		try {
			...
			console.error(
				"[WindowTitlebar] Failed to acquire Tauri window handle",
				error,
			);
			return null;
		}
		...
		currentWindow
			.onResized(() => {
				currentWindow
					...
					.catch((error) => {
						console.error(
							"[WindowTitlebar] Failed to sync maximized state",
							error,
						);
					});
			})
			...
			.catch((error) => {
				console.error(
					"[WindowTitlebar] Failed to attach resize listener",
					error,
				);
			});
	...
	const invoke = useCallback(
		(action: (appWindow: Window) => Promise<void>) => {
			if (!currentWindow) {
				console.error(
					"[WindowTitlebar] No Tauri window handle available for command",
				);
				return;
			}
	...
	return (
		/* biome-ignore lint/a11y/noStaticElementInteractions: Preserve native double-click maximize behavior; explicit window controls provide accessible alternatives. */
		<div
